...
.equ	IdTypeAVR=0x25		; must exists in "piccodes.ini"	
#define	max_flash 0x4000	; in WORDS, not bytes!!! (= 'max flash memory' from "piccodes.ini" divided by 2)
        
.equ	xtal	=	8000000	; you may also want to change: _HS_OSC _XT_OSC
.equ	baud    =	19200	; standard TinyBld baud rates: 115200 or 19200

.equ	Calib   =	0x93	; 8MHz Calibration value
...
; +---------+---------+--------+------------+------------+-----------+--------+------+
; |   0x32  |   0x25  | M328P  |  64 words  |  64 words  |  0x4000   |  1024  |  28  |
; +---------+---------+--------+------------+------------+-----------+--------+------+

    	;********************************************************************
    	;       Tiny Bootloader         ATMEGA328P              Size=132words
	;
	...
	;
	;	!!!!! Set Fuse Bit CKDIV8=1 and 8MHz Calibration value !!!!!
	;
	;	Please add the following line to piccodes.ini
	;
	;	$25, 2, ATMEGA 328P, $8000, 1024, default, default,
	;
	;********************************************************************
...
rs1rx:
		SBISI	UCSRA,RXC	; check Recive done
		RJMP	rs1rx		;
r1rx11:
