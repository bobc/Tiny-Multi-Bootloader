...
IdTypePIC = 0x6C		; Please refer to the table below, must exists in "piccodes.ini"	
...
mainl:
	...
	rcall 	Receive				;Upper
	movwf 	TBLPTRU
	movwf 	flag,BANKED			;(for EEPROM and CFG cases)
	rcall 	Receive				;Hi
	movwf 	TBLPTRH
...
ziieroare:					;CRC failed
	...
	btfss 	flag,6,BANKED			;is EEPROM data?
	bra 	noeeprom
...
noeeprom:
	clrf	FSR0L				;FSR0=buffer TOP
	btfss 	flag,7,BANKED			;is CFG data?
	bra 	noconfig
	TBLRD*-					; point to adr-1
lp_noeeprom:
	rcall	put1byte
	rcall 	Write
	decfsz	count,f,BANKED
	bra	lp_noeeprom
	bra 	waitwre
...
writebyte:
	rcall	put1byte
	decfsz 	count,f,BANKED			; 64/128 bytes?
	bra 	writebyte
...
rptc:
	...
	addwf 	crc,f,BANKED			;compute crc
	return
notrcv:
	decfsz 	cnt3,f,BANKED
	bra 	rptc
	decfsz 	cnt2,f,BANKED
	bra 	rpt3
	decfsz 	cnt1,f,BANKED
	bra 	rpt2
	;timeout:
way_to_exit:
	movlw	offset
